...
const UpgradePrompt: React.FC<UpgradePromptProps> = ({ 
    ...
    return (
      <div className={`flex items-center justify-between p-3 rounded-lg border transition-colors duration-300
        ...
        <div className="flex items-center space-x-3">
          <FaCrown className={darkMode ? 'text-yellow-400' : 'text-yellow-600'} />
  ...
  return (
    <div className={`relative p-6 rounded-xl border transition-colors duration-300
                   ${darkMode 
      ...
      {onDismiss && (
        <button
          ...
          className={`absolute top-4 right-4 p-1 rounded transition-colors
                    ${darkMode ? 'hover:bg-dark-600 text-dark-400' : 'hover:bg-gray-100 text-gray-500'}`}
        >
      ...
      <div className="flex items-start space-x-4">
        <div className={`p-3 rounded-full ${darkMode ? 'bg-yellow-600/20' : 'bg-yellow-100'}`}>
          <FaCrown className={`h-6 w-6 ${darkMode ? 'text-yellow-400' : 'text-yellow-600'}`} />
        </div>
        
        <div className="flex-1">
          <h3 className={`text-lg font-bold mb-2 ${darkMode ? 'text-dark-100' : 'text-gray-900'}`}>
            Unlock {feature}
          </h3>
          
          {description && (
            <p className={`text-sm mb-4 ${darkMode ? 'text-dark-300' : 'text-gray-600'}`}>
              {description}
            </p>
          )}
