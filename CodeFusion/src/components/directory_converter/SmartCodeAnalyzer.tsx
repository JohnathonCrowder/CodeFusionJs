...
import { TokenConfirmationModal, ApiKeyModal } from "../modals/ai-analysis";
...
const SmartCodeAnalyzer: React.FC<SmartCodeAnalyzerProps> = ({
  ...
  const { currentUser, getActiveApiKey, storeApiKey, getUserApiKeys } =
    useAuth();
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "quality" | "security" | "performance"
  >("overview");
  const [tokenCount, setTokenCount] = useState<number>(0);
  const [showTokenConfirmation, setShowTokenConfirmation] =
      ...
      if (!currentUser) {
        ...
        if (savedApiKey) {
          ...
          try {
            ...
            console.error(
              "Failed to initialize AI service with localStorage key:",
              error
            );
          }
        }
        ...
        if (firebaseKey) {
          ...
          if (savedApiKey) {
            ...
            try {
              ...
              setKeySource("firebase");
              console.log(
                "Successfully migrated API key from localStorage to Firebase"
              );
            } catch (error) {
  ...
  const runAnalysis = async () => {
    ...
    if (!aiService.isReady()) {
      setError(
        "AI service not properly initialized. Please reconfigure your API key."
      );
      setShowApiKeyModal(true);
      return;
    }
  ...
  const handleConfirmAnalysis = async () => {
    ...
    try {
      ...
      if (
        ...
        setError(
          "No compatible AI models found. Please ensure your OpenAI account has access to at least GPT-3.5-turbo and has billing enabled."
        );
      } else if (
        ...
        setError(
          "Model access issue detected. The system will automatically try alternative models. Please try again."
        );
      } else {
  ...
  return (
    <div
      className={`w-96 border-l flex flex-col h-full transition-colors duration-300
                   ${
                     darkMode
    ...
    >
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-1">
                ...
                <>
                  {keySource === "firebase" && currentUser && (
                    <FaCloud
                      ...
                      title="Key stored securely in cloud"
                    />
                  )}
              ...
              {currentUser && (
                ...
                >
                  <FaSync
                    className={`text-sm ${isLoadingKey ? "animate-spin" : ""}`}
                  />
                </button>
              )}
            </div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              {availableKeys.length > 0 && (
                <div className="flex items-center justify-between text-xs">
                  <span
                    className={darkMode ? "text-dark-400" : "text-gray-600"}
                  >
                    Keys Available:
                  </span>
                  <span
      ...
      >
        ...
        {error && (
          <div
            className={`mt-3 p-3 rounded-lg border-l-4 transition-colors duration-300
                         ${
                           darkMode
      ...
      <div className="flex-1 overflow-y-auto">
        {!analysis && !isAnalyzing ? (
          ...
          <div className="flex flex-col items-center justify-center h-full p-8 text-center">
            ...
            >
              Upload code files and run AI analysis to get insights about code
              quality, architecture, security, and performance.
            </p>
            <p
          ...
          <div className="p-4 space-y-4">
            ...
            {activeTab === "quality" && (
              <div className="space-y-4">
                ...
                >
                  <h4
                    className={`font-medium mb-3 flex items-center space-x-2 transition-colors duration-300
                                 ${
                                   darkMode ? "text-dark-200" : "text-gray-800"
                                 }`}
                  >
                ...
                >
                  ...
                  <ul className="space-y-1">
                    {analysis.codeQuality.improvements.map(
                      (improvement, index) => (
                        <li
                          key={index}
                          className={`text-sm transition-colors duration-300
                ...
                >
                  ...
                  <div className="space-y-2">
                    {analysis.bestPractices.following.length > 0 && (
                      <div>
                        <p
                          className={`text-xs font-medium mb-1 text-green-500`}
                        >
                          Following:
                        </p>
                        <ul className="space-y-1">
                          {analysis.bestPractices.following.map(
                            (practice, index) => (
                              <li
                                key={index}
                                className={`text-sm transition-colors duration-300
                    ...
                    {analysis.bestPractices.violations.length > 0 && (
                      <div>
                        ...
                        <ul className="space-y-1">
                          {analysis.bestPractices.violations.map(
                            (violation, index) => (
                              <li
                                key={index}
                                className={`text-sm transition-colors duration-300
              ...
              <div className="space-y-4">
                {analysis.security.issues.length > 0 ? (
                  ...
                  >
                    ...
                    <p className="text-sm">
                      The analysis didn't identify any obvious security
                      vulnerabilities.
                    </p>
                  </div>
                )}
              ...
              <div className="space-y-4">
                {analysis.performance.bottlenecks.length > 0 && (
                  ...
                  >
                    ...
                    <ul className="space-y-1">
                      {analysis.performance.bottlenecks.map(
                        (bottleneck, index) => (
                          <li
                            key={index}
                            className={`text-sm transition-colors duration-300
          ...
          <div className="flex items-center justify-center h-32">
            <div className="text-center">
              <FaSpinner
                className={`animate-spin text-2xl mx-auto mb-2 transition-colors duration-300
                                   ${
                                     darkMode
                                       ? "text-purple-400"
                                       : "text-purple-600"
