...
vi.mock('react-icons/fa', () => ({
  ...
  FaUpload: ({ className }: { className?: string }) => (
    <span data-testid="upload-icon" className={className}>UploadIcon</span>
  ),
...
describe('MainContent Component', () => {
  const mockAnonymizeContent = vi.fn((content: string) => content.replace(/test@example.com/g, '[EMAIL]'))
  ...
  describe('Rendering', () => {
    ...
    it('should display empty state when no files are provided', () => {
      ...
      expect(screen.getByText(/Upload files or a directory/)).toBeInTheDocument()
    ...
    it('should display nested file content', () => {
      ...
      expect(screen.getByText(/export const email = "test@example.com"/)).toBeInTheDocument()
    })
    ...
    it('should not display hidden files', () => {
      ...
      expect(screen.queryByText(/body { color: red; }/)).not.toBeInTheDocument()
    })
  ...
  describe('Copy Functionality', () => {
    it('should copy all visible content when copy button is clicked', async () => {
      ...
      expect(copiedContent).toContain('export const email = "test@example.com"')
  ...
  describe('Anonymization', () => {
    ...
    it('should anonymize content when isAnonymized is true', () => {
      ...
      expect(screen.getByText(/export const email = "\[EMAIL\]"/)).toBeInTheDocument()
    ...
    it('should show anonymization footer when active', () => {
      ...
      expect(screen.getByText('Privacy Protection Applied')).toBeInTheDocument()
      expect(screen.getByText(/Personal information has been anonymized/)).toBeInTheDocument()
    })
    ...
    it('should show privacy note in empty state when anonymized', () => {
      ...
      expect(screen.getByText(/Your personal information will be anonymized/)).toBeInTheDocument()
    })
  })
  ...
  describe('File Formatting', () => {
    it('should format files with separators', () => {
      ...
      const content = screen.getByText(/console.log/).closest('pre')?.textContent
  ...
  describe('Code Header', () => {
    ...
    it('should apply proper styling to code container', () => {
      ...
      const codeContainer = screen.getByText('Combined Project Files').closest('div')?.parentElement
  ...
  describe('Edge Cases', () => {
    it('should handle empty file content gracefully', () => {
      ...
      const pre = screen.getByText('Combined Project Files').closest('div')?.parentElement?.querySelector('pre')
      expect(pre).toBeInTheDocument()
    ...
    it('should handle files with special characters in content', () => {
      const specialFile = [{
        name: 'special.js',
        content: 'const regex = /[a-z]+/g; // Test <script>alert("XSS")</script>',
        visible: true,
        path: 'src/special.js'
      }]
